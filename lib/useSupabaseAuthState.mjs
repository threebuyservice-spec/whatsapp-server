...
export async function useSupabaseAuthState(supabase, sessionId, opts = {}) {
  const log = opts.log || (() => {});
  /** One mutex per session: serializes creds + all key files to avoid interleaved writes vs creds.update (reduces Bad MAC / corrupt auth). */
  const sessionIoMutex = new Mutex();

  const writeData = async (data, file) => {
    const fileKey = fixFileName(file);
    return sessionIoMutex.runExclusive(async () => {
      let serialized;
      try {
